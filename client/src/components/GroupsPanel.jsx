...
function GroupInfoModal({ group, onClose, onUpdated, onDeleted, onLeft }) {
  ...
  return (
    <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4 animate-fade-in" onClick={onClose}>
      <div className="bg-white rounded-2xl border border-gray-200 w-full max-w-lg max-h-[90vh] flex flex-col shadow-2xl animate-pop" onClick={e => e.stopPropagation()}>
        {/* Header */}
        <div className="flex items-center justify-between p-5 border-b border-gray-100">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-gray-400">{details?.member_count || '...'} members</p>
            </div>
          </div>
          <button onClick={onClose} className="p-1.5 text-gray-400 hover:text-gray-900 hover:bg-gray-100 rounded-xl transition-all">
            <X size={18} />
          </button>
        </div>
        ...
        <div className="flex-1 overflow-y-auto custom-scrollbar p-5 space-y-5">
          {loading ? (
            <div className="space-y-4">
              <div className="h-4 bg-gray-100 rounded-full animate-shimmer w-3/4" />
              <div className="h-4 bg-gray-100 rounded-full animate-shimmer w-1/2" />
              <div className="space-y-2 pt-4">
                {[1,2,3].map(i => <div key={i} className="h-12 bg-gray-100 rounded-xl animate-shimmer" />)}
              </div>
            </div>
          ) : (
            <>
              ...
              {editing ? (
                <div className="space-y-3">
                  <input value={editName} onChange={e => setEditName(e.target.value)} placeholder="Group name" maxLength={100}
                    className="w-full px-4 py-2.5 bg-gray-50 border border-gray-200 rounded-xl text-sm text-gray-900 placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-[#6C5CE7]/30" />
                  <textarea value={editDesc} onChange={e => setEditDesc(e.target.value)} placeholder="Description (optional)" maxLength={200} rows={2}
                    className="w-full px-4 py-2.5 bg-gray-50 border border-gray-200 rounded-xl text-sm text-gray-900 placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-[#6C5CE7]/30 resize-none" />
                  <div className="flex gap-2">
                    ...
                    <button onClick={() => setEditing(false)}
                      className="px-4 py-2 text-xs font-semibold text-gray-500 bg-gray-100 rounded-xl hover:bg-gray-200 transition-all">
                      Cancel
                    </button>
                  </div>
                </div>
                ...
                <div>
                  <p className="text-sm text-gray-700 leading-relaxed">{details?.description || <span className="text-gray-400 italic">No description</span>}</p>
                </div>
              )}
              ...
              <div className="flex flex-wrap gap-2">
                ...
                <button onClick={handleLeave}
                  className="flex items-center gap-1.5 px-3 py-2 text-xs font-semibold text-orange-600 bg-orange-50 hover:bg-orange-100 rounded-xl transition-all">
              ...
              <div>
                <div className="flex items-center justify-between mb-3">
                  ...
                  <span className="text-[10px] text-gray-400">{details?.member_count} total</span>
                </div>
                <div className="space-y-1">
                  {details?.members?.map(m => (
                    <div key={m.user_id} className="flex items-center gap-3 px-3 py-2.5 rounded-xl hover:bg-gray-50 transition-all group">
                      <div className="relative flex-shrink-0">
                        <AvatarCircle user={m} size="md" />
                        <span className={`absolute -bottom-0.5 -right-0.5 w-3 h-3 rounded-full border-2 border-white ${isOnline(m.last_seen_at) ? 'bg-emerald-500' : 'bg-gray-300'}`} />
                      </div>
                      ...
                      {isAdmin && m.role !== 'admin' && m.user_id !== myId && (
                        <div className="flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                          <button onClick={() => handlePromote(m.user_id)} title="Promote to admin"
                            className="p-1 text-gray-400 hover:text-amber-500 hover:bg-amber-50 rounded-lg transition-all">
                            <Crown size={13} />
                          </button>
...
function CreateGroupModal({ onClose, onCreated }) {
  ...
  return (
    <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4 animate-fade-in" onClick={onClose}>
      <div className="bg-white rounded-2xl border border-gray-200 w-full max-w-md p-6 shadow-2xl animate-pop" onClick={e => e.stopPropagation()}>
        <div className="flex items-center justify-between mb-5">
          ...
          <button onClick={onClose} className="p-1 text-gray-400 hover:text-gray-900 hover:bg-gray-100 rounded-xl transition-all"><X size={18} /></button>
        </div>
        <div className="space-y-4">
          <input value={name} onChange={e => setName(e.target.value)} placeholder="Group name" maxLength={100}
            className="w-full px-4 py-2.5 bg-gray-50 border border-gray-200 rounded-xl text-sm text-gray-900 placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-[#6C5CE7]/30 transition-all" />
          <input value={description} onChange={e => setDescription(e.target.value)} placeholder="Description (optional)" maxLength={200}
            className="w-full px-4 py-2.5 bg-gray-50 border border-gray-200 rounded-xl text-sm text-gray-900 placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-[#6C5CE7]/30 transition-all" />
          {friends.length > 0 && (
            <div>
              <p className="text-xs font-semibold text-gray-500 mb-2.5 uppercase tracking-wider">Add members ({selected.length} selected)</p>
              <div className="max-h-48 overflow-y-auto custom-scrollbar space-y-1 bg-gray-50 rounded-xl p-1.5">
                {friends.map(f => (
                  <button key={f.id} onClick={() => toggle(f.id)}
                    className={`w-full flex items-center gap-3 px-3 py-2 rounded-xl transition-all text-left ${
                      selected.includes(f.id) ? 'bg-[#6C5CE7]/10 border border-[#6C5CE7]/30 shadow-sm' : 'hover:bg-white border border-transparent'
                    }`}>
                    <AvatarCircle user={f} />
      ...
      <div className="px-4 py-3 border-b border-gray-100">
        <div className="flex items-center justify-between mb-2">
          <div className="flex items-center gap-2">
            ...
            {groups.length > 0 && <span className="text-[10px] text-gray-400 font-medium">({groups.length})</span>}
          </div>
        ...
        {loading ? (
          <div className="flex gap-2 py-1">
            {[1,2].map(i => <div key={i} className="h-8 w-20 bg-gray-100 rounded-xl animate-shimmer" />)}
          </div>
        ) : groups.length === 0 ? (
          <p className="text-[10px] text-gray-400 py-2 text-center">No groups yet — create one!</p>
        ) : (
            ...
            {groups.map(g => (
              <div key={g.id}
                ...
                <button onClick={() => onSelectGroup(g)} className="flex items-center gap-2.5 flex-1 min-w-0">
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-1.5">
                      <p className={`text-sm truncate ${activeGroupId === g.id ? 'font-bold text-gray-900' : 'font-semibold text-gray-700'}`}>
                        {g.name}
                      </p>
                    </div>
                    <div className="flex items-center gap-2">
                      <span className="text-[10px] text-gray-400">{g.member_count || 0} members</span>
                      {(g.last_message || g.last_at) && (
                        <>
                          <span className="text-[8px] text-gray-300">•</span>
                          <p className="text-[10px] text-gray-400 truncate max-w-[100px]">{g.last_message || (g.last_at ? timeAgo(g.last_at) : '')}</p>
                        </>
                      )}
                    </div>
                  </div>
                ...
                <button onClick={() => setShowInfo(g)}
                  className="p-1.5 text-gray-300 hover:text-[#6C5CE7] hover:bg-[#6C5CE7]/10 rounded-lg transition-all opacity-0 group-hover:opacity-100 flex-shrink-0">
                  <Info size={14} />
                </button>
              </div>
            ))}
