...
function MiniProfileCard({ user }) {
  return (
    <div className="absolute left-full ml-2 top-0 w-52 bg-white dark:bg-gray-900 rounded-xl border border-gray-100 dark:border-gray-800 shadow-xl p-3 z-50 pointer-events-none"
...
export default function MentionSuggestions({ suggestions, show, onSelect }) {
  const [hoveredUser, setHoveredUser] = useState(null);

  if (!show || !suggestions.length) return null;

  return (
    <div className="absolute z-50 bottom-full mb-2 left-0 w-64 bg-white dark:bg-gray-900 border border-gray-100 dark:border-gray-800 rounded-xl shadow-xl overflow-visible"
      style={{ boxShadow: '0 8px 30px rgba(0,0,0,0.14)', animation: 'fadeInUp 0.15s ease' }}>
      <div className="px-3 pt-2 pb-1 flex items-center gap-2 border-b border-gray-50 dark:border-gray-800">
      ...
      <div className="max-h-48 overflow-y-auto">
        {suggestions.map(user => (
          <button
            ...
            className="relative flex items-center gap-2.5 w-full px-3 py-2.5 hover:bg-indigo-50 dark:hover:bg-indigo-900/20 transition-colors text-left group"
          >
