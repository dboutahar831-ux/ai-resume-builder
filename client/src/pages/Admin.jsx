  ...
  return (
    <div className="bg-[#151921] rounded-2xl border border-[#21262E] p-5">
      <div className="flex items-center gap-3 mb-2">
        ...
        <p className="text-2xl font-bold text-[#E8ECF1]">{value ?? '—'}</p>
      </div>
    ...
    return (
      <Layout>
        <div className="max-w-2xl mx-auto py-20 text-center">
          ...
          <h1 className="text-xl font-bold text-[#E8ECF1] mb-2">Admin Access Required</h1>
          <p className="text-sm text-[#8B95A5] mb-6">You don't have permission to access this page.</p>
  ...
  return (
    <Layout>
      <div className="max-w-6xl mx-auto space-y-6">
        {/* Header */}
        <div className="bg-[#151921] rounded-2xl border border-[#21262E] p-5 flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-lg font-bold text-[#E8ECF1]">Admin Dashboard</h1>
              <p className="text-xs text-[#8B95A5]">Site management &amp; overview</p>
            </div>
          </div>
          <button onClick={load} disabled={loading}
            className="flex items-center gap-1.5 px-3 py-1.5 text-xs font-medium text-[#8B95A5] hover:text-[#E8ECF1] hover:bg-[#1A1F2B] rounded-xl transition-all">
          ...
          {['overview', 'users', 'activity'].map(t => (
            <button key={t} onClick={() => setTab(t)}
              className={`px-4 py-2 rounded-xl text-xs font-semibold transition-all capitalize ${
                tab === t ? 'text-white' : 'text-[#8B95A5] bg-[#1A1F2B] hover:bg-[#21262E]'
              }`}
        ...
        {tab === 'users' && (
          <div className="bg-[#151921] rounded-2xl border border-[#21262E] overflow-hidden">
            <div className="overflow-x-auto">
              <table className="w-full text-sm">
                <thead>
                  <tr className="border-b border-[#21262E] text-[#8B95A5] text-xs uppercase tracking-wider">
                ...
                <tbody className="divide-y divide-[#21262E]">
                  {users.map(u => (
                    <tr key={u.id} className="hover:bg-[#1A1F2B] transition-colors">
                      ...
                      <td className="px-5 py-3 text-[#8B95A5]">{u.email}</td>
                      ...
                      <td className="px-5 py-3 text-center text-[#8B95A5]">{u.post_count}</td>
                      <td className="px-5 py-3 text-center text-[#8B95A5]">{u.resume_count}</td>
                      <td className="px-5 py-3 text-center text-[#5A6375] text-xs">{new Date(u.created_at).toLocaleDateString()}</td>
        ...
        {tab === 'activity' && activity && (
          <div className="bg-[#151921] rounded-2xl border border-[#21262E] p-5">
            <h2 className="text-sm font-bold text-[#E8ECF1] mb-3">Recent Activity</h2>
            <div className="space-y-3">
              {activity.posts.map(p => (
                <div key={p.id} className="flex items-start gap-3 pb-3 border-b border-[#21262E] last:border-0">
                  ...
                  <div className="flex-1 min-w-0">
                    <p className="text-sm text-[#E8ECF1]">
                      ...
                      <span className="text-[#8B95A5]"> posted </span>
                      <span className="text-[#8B95A5]">{(p.content || '').slice(0, 100)}{(p.content || '').length > 100 ? '…' : ''}</span>
                    </p>
                    <p className="text-[10px] text-[#5A6375] mt-0.5">{new Date(p.created_at).toLocaleString()}</p>
                  </div>
                </div>
              ))}
            </div>
