...
function StatusDot({ lastSeenAt }) {
  ...
  return <span className={`absolute bottom-0 right-0 w-2.5 h-2.5 rounded-full border-2 border-white ${online ? 'bg-emerald-500' : 'bg-gray-300'}`} />;
}
...
export default function Messages() {
  ...
  const loadConversations = useCallback(async () => {
    const [c, f] = await Promise.all([api.get('/messages/conversations'), api.get('/friends')]);
  ...
  const handleInputChange = (e) => {
    ...
    if (activeUser) {
      const now = Date.now();
      if (now - lastTypingPost.current > 1500) {
                  ...
                  <div className={`flex ${isMe ? 'justify-end' : 'justify-start'} mb-0.5`}>
                    <div className="max-w-xs lg:max-w-sm">
                      ...
                      <div className={`px-4 py-2.5 rounded-2xl text-sm leading-relaxed break-words relative ${
                        ...
                        {/* Check icon inside bubble (bottom right) */}
                        {isMe && (
                      ...
                      {/* Status text below last sent message */}
                      {isLastMySent && (
          ...
          {activeUser && (
            <div className="border-t border-gray-100 bg-white flex-shrink-0">
              ...
              {msgImage && (
                <div className="px-4 pt-3 flex items-start gap-2">
                  <div className="relative inline-block">
                    <img src={msgImage} alt="preview" className="h-20 rounded-xl object-cover border border-gray-200" />
              ...
              {pendingVoice && !recording && (
                <div className="px-4 py-2 flex items-center gap-3 bg-indigo-50 border-b border-indigo-100">
              ...
              {!recording && (
                <div className="px-4 py-3 flex gap-2 items-end">
                  ...
                  <textarea
                    ...
                    rows={1}
                    className="flex-1 px-4 py-2.5 border border-gray-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-indigo-300 bg-white resize-none overflow-hidden leading-relaxed"
